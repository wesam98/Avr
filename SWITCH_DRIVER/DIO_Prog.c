	...
	if(Copy_u8PortID <= DIO_PORTD && Copy_u8PinID <= DIO_PIN7 && Copy_u8PinDirection <= BIT_MASK){
		switch(Copy_u8PortID)
				...
				{  DDRA &= ~(BIT_MASK<< Copy_u8PinID);
				   DDRA |= (BIT_MASK<< Copy_u8PinDirection);
				   break;
				}
				case(DIO_PORTB):
					{  DDRB &= ~(BIT_MASK<< Copy_u8PinID);
					   DDRB |= (BIT_MASK<< Copy_u8PinDirection);
					   break;
					}
				case(DIO_PORTC):
					{  DDRC &= ~(BIT_MASK<< Copy_u8PinID);
					   DDRC |= (BIT_MASK<< Copy_u8PinDirection);
					   break;
					}
				case(DIO_PORTD):
					{  DDRD &= ~(BIT_MASK<< Copy_u8PinID);
					   DDRD |= (BIT_MASK<< Copy_u8PinDirection);
					   break;
					}
				 Local_enuErrorState=ES_OK;
				}
			...
			switch(Copy_u8PortID)
					...
					case(DIO_PORTA):
					    {  PORTA &= ~(BIT_MASK<< Copy_u8PinID);
					   	   PORTA |= (BIT_MASK <<Copy_u8PinValue);
					   	   break;
					    }
					case(DIO_PORTB):
						{  PORTB &= ~(BIT_MASK<<Copy_u8PinID);
						   PORTB |= (BIT_MASK<< Copy_u8PinValue);
						   break;
						}
					case(DIO_PORTC):
						{  PORTC &= ~(BIT_MASK<< Copy_u8PinID);
						   PORTC |= (BIT_MASK<< Copy_u8PinValue);
						   break;
						}
					case(DIO_PORTD):
						{  PORTD &= ~(BIT_MASK<< Copy_u8PinID);
						   PORTD |= (BIT_MASK<< Copy_u8PinValue);
						   break;
						}
					 Local_enuErrorState=ES_OK;
					}
